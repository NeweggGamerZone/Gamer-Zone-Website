...
## Live square Weekly Lineup board (`#week .eu-board`)

Added 2026-09-01 at Eric's request: the real, visitor-facing Weekly Lineup
board on `index.html`/`events.html` (not just the social-export screenshots)
now reads as a filled square, matching the look of the square export. Scoped
specifically to `#week .eu-board` — `screenshot-monthly-calendar.html`'s
`#month .eu-board` (a deliberately taller list layout) is untouched.

- **The square floor is set from JS, not a CSS `cqw` value** —
  `squareBoardMinHeight()` in `assets/js/event-update.js` measures
  `getBoundingClientRect().width` and sets `min-height` to match, re-run on
  resize. `.eu-board` already establishes its own
  `container-type: inline-size` context (for its internal `cqw` type
  scale — see "One shared container width" above); a `cqw` length used on
  that *same* element can't resolve against itself, so it silently falls
  back to an ancestor container instead. In testing that produced a
  920×1400 box at a 1400px viewport instead of ~920×920. Measuring the
  real rendered width in JS sidesteps the circularity entirely.
- **This is `min-height`, not `aspect-ratio`** — per the rule two sections
  up, a genuine floor that still lets the box grow for real content. A
  busy week's real event list still pushes the board taller than square;
  a light week just gets its content centered inside a square footprint
  instead of top-anchored with dead space below (`justify-content:center`
  on `#week .eu-board` in `style.css` does that centering).
- **`weeklyThemes[].icon` (`data/events.json`) is what fills that space**
  on a light week — reusing the per-week icon slot that already existed
  for this purpose, not a new bespoke fill mechanism. Riot Games Week
  (2026-09-01 to 09-05, a Labor Day closure week with only one event) uses
  `assets/calendar/LineupAssets/us-flag-metallic.png` this way.
- **Known follow-up, not fixed here:** `html.board-mode`'s own
  `.eu-bottom-group{margin-top:auto}` (meant for the old, apparently-
  never-committed `weekcapture.js` square-padding pipeline) still pins the
  icon flush to the very bottom of a board-mode export, which now fights
  with the live square's centered look and leaves an ugly gap in the
  square/16:9 social exports specifically. The *live* page is unaffected
  (that rule is board-mode-only). Reconcile board-mode's export layout
  with this live-square behavior before next relying on the square/16x9
  weekly export images.
...
400/640/900/1400px with both the real Sep 2026 data and an artificially
long stress-test theme name — confirmed zero page-level overflow and, via
`scrollWidth`/`clientWidth`/`getBoundingClientRect` measurements at each
nesting level (not just a visual screenshot check), that the title
element's own box no longer exceeds its container. If a future masthead
or eyebrow-style component nests flex containers more than one level
deep, don't assume `min-width:0` on the outermost item is enough — check
`scrollWidth` vs. the true container width at every level, not just the
one you touched.
