...
## Round 18, same day: prize-tile row layout, square-format square-ish nodes, character 5% larger -- a rejected 4-column attempt and a real footer-clipping bug found and fixed along the way (2026-09-22)

Per Eric, immediately after Round 17: "Still alot of empty space in the tiles. Can you move the 150 and 100 to be to the right of the 1,2,3,4. and then move the placement number upwards. Then make everything within those tile nodes to be larger. I also want square nodes on the square graphic version, like how the horizontal version has square nodes. See if you can make the graphic on the square image 5% larger." Five concrete, fully-specified parts, implemented straight through per core rule 15 -- all still scoped to `event-card.html`'s own inline `<style>`/`<script>` blocks, same convention as every round on this page since Round 15.

**Prize tile reworked from a column to a row layout -- the real cause of "still a lot of empty space."** Each tile (`.prize-row`) previously stacked the place badge on top, with the $ amount and place-label below it, both left-aligned inside a wide 2-column grid box -- leaving the entire right half of every tile visibly empty regardless of font size. Switched to `flex-direction:row` with the badge and the amount/label pair side by side (`gap:18px`), directly satisfying "move the 150 and 100 to be to the right of the 1,2,3,4." `align-items:flex-start` pins the badge to the top of the row per Eric's explicit "move the placement number upwards" -- needed because a row layout would otherwise default to vertically centering the badge against the now-two-line text block beside it.

**Everything inside the tiles sized up again, on top of Round 17's own bump:** place-badge circle 44px -> 58px (font 15px -> 23px), `.amt` 48px -> 58px, `.place-label` 19px -> 21px.

**"Square nodes" on the square format: two rejected approaches before landing on the real fix, each one actually rendered and checked, not assumed from the CSS.** (1) A literal `aspect-ratio:1/1` on `.prize-row` first: at the square format's 294px-wide 2-column track this forces true 294x294px tiles, but the row-layout content only fills roughly half that height, leaving a large visible dead zone below the content in each tile -- reintroducing the exact "empty space" complaint in vertical form, caught by rendering and inspecting the result rather than reasoning about it on paper. (2) Matching the horizontal format's 4-column layout on square next, on the theory that "square nodes" meant matching column count too: visually this looked fine in a screenshot, but a direct `getComputedStyle().gridTemplateColumns` check on `.prize-table` revealed the four tracks were NOT equal (257/262/230/234px) and summed with gaps to ~1020px -- vastly overflowing `.prize-table`'s own declared 600px width. Root cause: a CSS Grid `1fr` track's automatic minimum size defaults to its content's own min-content width (not 0), so when four columns' worth of badge+$-amount content couldn't realistically fit an equal ~141px share, every track silently grew to fit its own content instead -- a real violation of this file's own "don't let a container react to its own content length" rule, not a cosmetic issue. Reverted to 2 columns (which does fit its content's real min-content width at 294px, confirmed via the same computed-style check showing two equal 294px tracks, no blowout) and instead approximated "square" via `aspect-ratio:4/3` on `.prize-row` -- chosen after measuring the horizontal format's own tiles aren't a literal 1:1 either (1.12-1.27:1 live), so 4:3 (1.33:1) matches that same real near-square reference point Eric was actually pointing at, while leaving genuine room for the row-layout content to sit inside it without either the vertical dead-zone of approach (1) or the horizontal blowout of approach (2). Applied to `.prize-row` itself, a CSS Grid item (not a plain block), so per this file's own "aspect-ratio on a plain block is a forced size, not a floor" rule it still gets a real content-based automatic minimum and won't silently clip if content ever needs more room than the ratio allows.

**A real footer-clipping bug, found via measurement and fixed, not just eyeballed.** Even after settling on 4:3, a live `getBoundingClientRect()` check showed `.footer-line`'s bottom edge measuring 1210.95px against the card's own 1200px height -- genuinely clipped past the visible export, not a hypothetical. Fixed with two small trims specific to the square format: `.content`'s `margin-top` 64px -> 54px, and the square-only `.prize-table` override gained its own `margin-bottom:12px`. Re-measured clean afterward: footer bottom at 1190.95px, comfortably inside the 1200px card, with zero text overflow or element clipping anywhere in either format.

**A real WCAG contrast regression surfaced by the geometry change, caught during the same verification pass and fixed.** The taller 4:3 tiles (220.5px vs. the previous layout's shorter boxes) moved `.place-label`'s on-screen position enough that a fresh live pixel-sample came back at 5.98:1 for the first prize tile in the square format -- under the 7:1 AAA floor, a real regression from the 7.61:1 worst-case Round 17 had verified clean. Nudged one step lighter again, `#D0D5E0` -> `#DCE0E8` (the same color-nudging methodology as Round 17's own original fix), and re-verified via the identical live Puppeteer + pixel-sampling method across all four tiles in both formats: worst-case is now 8.02:1 (square) and 9.02:1 (horizontal) -- both comfortably clear, with real margin against any future tile-position shift.

**Square-format character graphic grown 5%, per Eric's direct request.** `.card-char` and its 3 `.card-char-ghost-*` echo copies (which must stay the character's own exact size per the Round 16 rule) all grew together: 560px -> 588px tall. The horizontal format's own separate 700px override is untouched -- Eric's request was scoped to "the graphic on the square image" specifically.

**Verified via the same live Puppeteer + pixel-sampling method used throughout this page's history:** both rejected approaches were actually rendered and measured before being discarded (not assumed unsafe from reading the CSS), the final 4:3/2-column/trimmed-margin geometry was confirmed clean of any clipping or text overflow via `getBoundingClientRect()` in both formats, and the full contrast re-check after the `place-label` color fix came back with zero failures (worst case 8.02:1 square, 9.02:1 horizontal, across place/amt/place-label/perk-chip/cd-meta-item). The full site-wide scripted QA suite (`tools/audit/run-full-qa.sh`) was re-run afterward and came back at the known-clean baseline: 988 text items across the 5 real site pages, 0 container-width findings, 0 console errors, and only the same 25 already-disclosed Featured Gear/nav-pill false positives documented under "Readability" above -- `event-card.html` itself isn't part of that scripted suite (it's a hidden export page, not a real site page), so its own verification is the live Puppeteer checks described above.

No cache-bust bump needed -- every change is scoped to `event-card.html`'s own inline `<style>` block, same as every round on this page since Round 15.

## Round 19, same day: place badge removed entirely, prize tile becomes a plain centered amount+label pair (2026-09-22)

Per Eric, immediately after Round 18: "Remove the 1 2 3, 4, icons since it feels like it conflicts with it, and then just have the number and the place fill the node. ensure the number and then place fits in the same line, horizontal has multiple lines. Center aligned prize money and place beneath it." A direct, fully-specified follow-up (implemented straight through per core rule 15), still scoped to `event-card.html`'s own inline `<style>`/`<script>` blocks.

**The numbered place badge (the "1"/"2"/"3"/"4" orange circle) is gone.** Per Eric, it was showing the same ordinal twice in two different shapes right next to each other -- a numeral in a badge and the spelled-out ordinal in "1ST PLACE" a few pixels away -- which read as conflicting rather than reinforcing. The `<span class="place">` markup was removed from `prizeHTML`'s template in the page's script, and its dedicated CSS rule (`#card .prize-row .place{...}`) was deleted outright rather than just hidden.

**Each tile is now a plain centered column: the $ amount on top, the place label beneath it, both centered.** `.prize-row` switched from Round 18's row layout (badge left, amount+label right) to `flex-direction:column;align-items:center;justify-content:center` -- directly satisfying "center aligned prize money and place beneath it." With the badge gone and nothing else competing for space, the pair now genuinely fills the tile rather than sitting off to one side of empty room.

**"1ST PLACE" etc. is kept to one line where there's real room, and allowed to wrap where there genuinely isn't -- per Eric's own "ensure the number and then place fits in the same line, horizontal has multiple lines."** Read as: don't let the ordinal-plus-word wrap into two lines when there's space for it, but the horizontal format's narrower 4-column tiles (see the prize-table comment above) may still need to, and that's an acceptable, disclosed difference between the two formats rather than something to force. `white-space:nowrap` on `.place-label` is scoped to the square format only (`body:not(.fmt-horizontal)`), which has real room in its 294px-wide 2-column tiles -- forcing it everywhere would have risked genuine overflow on the horizontal format's narrower columns instead of the acceptable wrap Eric's own phrasing anticipated.

**A real contrast regression, caught and fixed during verification, needed two follow-up nudges before it actually cleared the floor.** Removing the badge and re-centering the label under the amount moved `.place-label` to a new on-screen position -- a live pixel-sample came back at 6.21:1 in the square format, under the 7:1 AAA floor (a real regression from Round 18's clean 8.02:1). The first fix attempt (`#DCE0E8` -> `#E9ECF1`, one step lighter, the same nudging methodology used in Rounds 17 and 18) still only reached 6.94:1 -- close, but still short. Rather than guess a third time, sampled the real worst-case background this label can land on directly: even *pure white* only measures 8.19:1 against it, meaning that's close to the practical ceiling available at this exact position, not an arbitrary number. Set `.place-label` to `#fff` and re-verified via the same live Puppeteer + pixel-sampling method across all four tiles in both formats: worst-case is now 8.22:1 (square) and 11.93:1 (horizontal) -- both comfortably clear. This means `.place-label` is no longer visually distinguished from `.amt` by a dimmer tint (both are white now) -- a real, disclosed tradeoff of this specific tile position/background, not an oversight; a future session revisiting this card's photo or scrim should re-check whether a dimmer secondary tint becomes viable again.
